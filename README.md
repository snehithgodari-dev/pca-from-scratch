...
## Results
 ...
 1. Match vs. `sklearn.decomposition.PCA` (sign-aligned) |                  max abs diff `< 1e-10` (machine precision)
 2. Embedding reduction                                                     128-dim → 12-dim, **86.3%** variance retained 
 3.Clustering quality (silhouette score) |                                  0.607 → 0.864(improved — noise dims removed) 
 4.Inference throughput (20,000 queries, cluster assignment)    |           ~4–5x faster on reduced representation 
 5. Core implementation size        |                                        19 lines of NumPy (`fit` + `transform`) 
